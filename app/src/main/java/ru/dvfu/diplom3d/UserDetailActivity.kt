...
import ru.dvfu.diplom3d.api.UpdateFlagRequest
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (userId != -1) {
            CoroutineScope(Dispatchers.Main).launch {
                loadAndDisplayUser(true)
            }
        }
    }

    private suspend fun loadAndDisplayUser(showLoader: Boolean) {
        if (showLoader) fullScreenLoading.visibility = View.VISIBLE
        ...
        try {
            ...
            if (showLoader) fullScreenLoading.visibility = View.GONE
        }
    }

    private fun setupUI(user: UserResponse) {
        content.removeAllViews()
        val api = RetrofitInstance.getApiService(getSharedPreferences("app_prefs", MODE_PRIVATE).getString("server_url", "")!!, this)
        ...
        if (currentUser != null) {
            val isCurrentUserSuperuser = currentUser.is_superuser
            val canStaffEditActive = currentUser.is_staff && !user.is_staff && !user.is_superuser
            
            val canSeeAndEditActive = isCurrentUserSuperuser || canStaffEditActive

            divider.visibility = if (canSeeAndEditActive) View.VISIBLE else View.GONE
            activeSwitch.visibility = if (canSeeAndEditActive) View.VISIBLE else View.GONE
            ...
            activeSwitch.isEnabled = canSeeAndEditActive
            ...
            staffSwitch.visibility = View.GONE
            superuserSwitch.visibility = View.GONE
        }
        ...
        activeSwitch.setOnCheckedChangeListener { _, isChecked ->
            if (activeSwitch.isPressed) {
                CoroutineScope(Dispatchers.Main).launch {
                    ...
                    try {
                        api.changeIsActive(user.id, UpdateFlagRequest(isChecked))
                        loadAndDisplayUser(false)
                    } finally {
                        fullScreenLoading.visibility = View.GONE
                    }
                }
        ...
        staffSwitch.setOnCheckedChangeListener { _, isChecked ->
            if (staffSwitch.isPressed) {
                CoroutineScope(Dispatchers.Main).launch {
                    fullScreenLoading.visibility = View.VISIBLE
                    try {
                        api.changeIsStaff(user.id, UpdateFlagRequest(isChecked))
                        loadAndDisplayUser(false)
                    } finally {
                        fullScreenLoading.visibility = View.GONE
                    }
        ...
        superuserSwitch.setOnCheckedChangeListener { _, isChecked ->
            if (superuserSwitch.isPressed) {
                CoroutineScope(Dispatchers.Main).launch {
                    fullScreenLoading.visibility = View.VISIBLE
                    try {
                        api.changeIsSuperuser(user.id, UpdateFlagRequest(isChecked))
                        loadAndDisplayUser(false)
                    } finally {
                        fullScreenLoading.visibility = View.GONE
                    }
